...
class STDEPCalculator:
    ...
    @staticmethod
    def detect_continuation_state(run_folder: str, convergence_threshold_override: float = None,
                                  snapshot_increment_override: int = None) -> Tuple[int, int, Dict[str, Any], bool]:
        """
        ...
        Args:
            ...
            convergence_threshold_override: If provided, use this threshold instead of the saved one
            snapshot_increment_override: If provided, use this increment instead of the saved one
        ...
        if convergence_file.exists():
            ...
            if len(free_energies) >= 2:
                convergence_threshold = convergence_threshold_override if convergence_threshold_override is not None \
                    else original_params.get('convergence_threshold', 0.001)
                convergence_factor = original_params.get('convergence_factor', 10.0)
                snapshot_increment = snapshot_increment_override if snapshot_increment_override is not None \
                    else original_params.get('snapshot_increment', 100)
        ...
        if convergence_file.exists() and len(free_energies) >= 2:
            conv_threshold = convergence_threshold_override if convergence_threshold_override is not None \
                else original_params.get('convergence_threshold', 0.001)
    ...
    if args.continue_run:
        ...
        try:
            continue_start_step, continue_snapshots, original_params, already_converged = \
                STDEPCalculator.detect_continuation_state(
                    args.continue_run,
                    convergence_threshold_override=args.convergence_threshold,
                    snapshot_increment_override=args.snapshot_increment)
