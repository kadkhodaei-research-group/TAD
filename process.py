...
parser.add_option("--latticefile", help="Specify lattice output file defalut infile.lattice",
                  action="store", type="string", dest="latticefile", default="infile.lattice")
...
parser.add_option("--npt", help="Parse an NPT simulation",
                  action="store_true", dest="npt", default=False)
...
print ("latticefile:", options.latticefile)
...
rePSTRESS  = re.compile(r"\s*PSTRESS\s*=\s*(\d+\.?\d+)\s+pullay stress")
...
if(options.npt): latticefile = open(options.latticefile,'w')
...
while potim == inf or nions == inf or lattice[1,1] == inf or tebeg == inf or sigma == inf or ismear == inf:
    ...
    if(rePSTRESS.match(line)):
        m = rePSTRESS.search(line)
        pstress = m.group(1)
        print ("PSTRESS: ", pstress)
...
for file in args:
    ...
    while 1:
        ...
        # Match the lattice vectors
        if(options.npt and reLatt2.match(line)):
            for i in range(0,3):
                line = f.readline()
                m = reVect2.search(line)
                lattice[i] =[float(m.group(1)),float(m.group(2)),float(m.group(3))]
        ...
        # matcha i energi blocket
            ...
            if ( filestep > skip ):
                ...
                if(options.magnetic):
                    ...
                    magtmp=""
                # Also write the lattice degrees of freedom
                if(options.npt):
                    latticefile.write("{0: .11f} {1: .11f} {2: .11f}\n".format(lattice[0][0], lattice[0][1], lattice[0][2]))
            ...
            for i in range(nions):
                ...
                if ( filestep > skip ):
                    ...
                    forcefile.write("{0: .7f} {1: .7f} {2: .7f}\n".format(line[3],line[4],line[5]))
...
statfileplot.write("set terminal GNUPLOTTERM size 1200,800 font \"CMU Serif,10\"\n")
